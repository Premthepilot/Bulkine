...
export const addFoodLog = async (foodEntry, date = null) => {
  try {
    ...
    console.log('[addFoodLog] User:', user ? { id: user.id, email: user.email } : 'null');

    if (!user) {
      console.error('[addFoodLog] User is null - not authenticated');
      throw new Error('User not authenticated');
    }
    ...
    console.log('[addFoodLog] Payload:', JSON.stringify(payload, null, 2));
    ...
    if (error) {
      console.error('[addFoodLog] Supabase error:', {
    ...
    console.log('[addFoodLog] Success! Entry added:', data);

    // Update total calories for the day in users_data table
    await updateDailyCalories(date);

    return data;
  } catch (error) {
    console.error('[addFoodLog] Error:', error.message || error);
    throw error;
  }
};
