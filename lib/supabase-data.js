...
export const upsertUserProfile = async (profileData) => {
  try {
    ...
    console.log('[upsertUserProfile] Incoming profileData:', JSON.stringify(profileData, null, 2));

    const user = await getCurrentUser();

    // DEBUG: Log user info
    console.log('[upsertUserProfile] User from auth:', user ? { id: user.id, email: user.email } : 'null');
    ...
    // Validate required fields are not undefined
    const requiredFields = ['body_type', 'main_goal', 'height', 'weight', 'goal_weight'];
    const missingFields = requiredFields.filter(field => profileData[field] === undefined);
    if (missingFields.length > 0) {
      console.warn('[upsertUserProfile] Warning - missing fields:', missingFields);
    }
    ...
    console.log('[upsertUserProfile] Full payload to upsert:', JSON.stringify(payload, null, 2));
    ...
    console.log('[upsertUserProfile] Supabase response - data:', data);
    console.log('[upsertUserProfile] Supabase response - error:', error);

    if (error) {
      console.error('[upsertUserProfile] Supabase error details:', {
    ...
    console.log('[upsertUserProfile] Success! Saved profile:', data);
    return data;
  } catch (error) {
