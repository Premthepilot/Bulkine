...
export const getUserProfile = async () => {
  ...
  try {
    ...
    // Use upsert to handle race conditions (multiple concurrent requests)
    if (!data) {
 ...
 * Called after authentication to set up profile
...
export const ensureUserProfile = async () => {
  ...
  try {
    // Get user's name from auth metadata (Google, email signup, etc)
    const userName = user.user_metadata?.name || user.email?.split('@')[0] || 'User'

    // Upsert ensures no duplicate key errors even with concurrent requests
    const { data, error } = await supabase
      .from('users_data')
      .upsert({
        user_id: user.id,
 ...
 * Use this after food/weight operations to sync UI with DB state
 * Ensures UI always reflects current database values
 */
...
export const updateFoodLog = async (id, { quantity, calories_per_unit }) => {
  ...
  try {
    ...
    if (!calories_per_unit || calories_per_unit <= 0) throw new Error('Calories per unit must be greater than 0')

    const total_calories = calories_per_unit * quantity

    // Update with verification that entry belongs to user (RLS + manual check)
    const { data, error } = await supabase
      .from('food_logs')
      .update({
        quantity,
    ...
    if (error) throw error
    if (!data) throw new Error('Food log entry not found or access denied')
...
export const addWeightEntry = async (weight, unit = 'kg') => {
  ...
  try {
    ...
    if (error) throw error
    if (!data) throw new Error('Failed to save weight entry')
 ...
 * Ensures weight data stays accurate after corrections
 */
export const deleteWeightEntry = async (recordedDate) => {
  const user = await getCurrentUser()
  if (!user) throw new Error('User not authenticated')
...
export const getLatestWeight = async () => {
  ...
  try {
    ...
    // Ensure weight is parsed as number
    if (data) {
      return {
        ...data,
